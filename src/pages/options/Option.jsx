...
import { fetchOptionsBySchoolId } from "../../feautres/option/optionSlice";
...
const Option = () => {
  const { options, loading } = useSelector((state) => state.option);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { id } = useParams();

  useEffect(() => {
    dispatch(fetchOptionsBySchoolId(id));
  }, [dispatch, id]);
          ...
          <table className="table-auto w-full dark:text-gray-300">
            ...
            <thead className="text-xs uppercase text-white dark:text-gray-500 bg-sky-900 dark:bg-gray-700 dark:bg-opacity-50 rounded-sm">
              <tr>
                ...
                <th className="p-2 text-left">Opion</th>
              ...
              {loading ? (
                ...
                options.map((item, i) => (
                  <tr key={item._id}>
                    ...
                    <td className="p-2">{item?.cycle.name}</td>
                     <td className="p-2">{item.optionName}</td>
