...
function DashboardCard01() {
  const totalSchools = 24780; // valeur dynamique possible
  ...
  // Définir la couleur dynamique selon la valeur
  const getBadgeColor = () => {
    if (growth >= 50) return 'text-green-900 bg-green-400/30';
  ...
  return (
    <div className={`flex flex-col col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow rounded-xl p-4 sm:p-5 transform transition-all duration-700 ${showCard ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
      ...
      <div className="flex items-center">
        <div className={`text-2xl sm:text-3xl font-bold mr-3 transition-colors duration-700 ${growth > 50 ? 'text-green-800' : 'text-gray-800 dark:text-gray-100'}`}>
          <CountUp start={0} end={totalSchools} duration={2.5} separator="," />
        </div>

        {showArrow && (
          <div className={`flex items-center text-xs sm:text-sm font-medium px-2 py-1 rounded-full transition-all duration-500 ${getBadgeColor()} transform ${isPositive ? 'translate-y-0' : 'translate-y-0'}`}>
